...
function Server(configuration) {
  ...
  var staticsPath = __dirname + "/statics/";
  debug("Serve static ", staticsPath);
  this.connect.use(serve_static(staticsPath));
}
...
Server.prototype._processDescription = function(request, response, next) {
  ...
  // C'est une freebox !
  if (/fbxlanbrowser/i.exec(agent)) {
    return next();
  }

  debug("Send nofreebox description");

  response.writeHead(200, {
    'Content-Type' : 'text/xml'
  });

  fs.createReadStream(__dirname + "/statics/description-nofreebox.xml").pipe(
      response);
}
...
Server.prototype.listen = function(callback) {
  ...
  server.listen(function(error) {
    ...
    var ssdConfig = {
      ...
      ssdpSig : "Node/" + process.versions.node + " FreeboxJasmin/1.0 " +
