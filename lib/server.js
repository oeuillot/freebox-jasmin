...
function Server(configuration) {
  ...
  if (debug.enabled) {
    var logger = function(request, response, next) {
      debug("Request: " + request.url + " from " + request.address());
      next();
    };
  ...
  debug("Serve static ", this._staticsPath);
...
Server.prototype._processDescription = function(request, response, next) {
  ...
  fs.createReadStream(Path.join(this._staticPath, "description-nofreebox.xml"))
      .pipe(response);
}
